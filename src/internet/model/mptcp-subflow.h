...
//  AvailableWindow(void);      // Return unfilled portion of window
   ...
   * \return true
  */
  virtual bool UpdateWindowSize (const TcpHeader& header) override;
...
//  uint8_t addrId,
  ...
  */
  ...
  \param mapping
  **/
  virtual bool DiscardAtMostOneTxMapping(SequenceNumber64 const& dack,
...
//  DoForwardUp(Ptr<Packet> packet, Ipv4Header header, uint16_t port, Ptr<Ipv4Interface> incomingInterface);
  ...
  *
  */
  virtual Ptr<Packet> ExtractAtMostOneMapping(uint32_t maxSize, bool only_full_mappings, SequenceNumber64& dsn);
...
//  ForkAsSubflow(void) = 0;
  ...
  * This should
  */
  virtual void NewAck(SequenceNumber32 const& ack, bool resetRTO) override;

  virtual void TimeWait();

  virtual void DoRetransmit() override;
...
//  bool ReadOptions(Ptr<Packet> pkt, const TcpHeader& mptcpHeader);
   ...
   */
  virtual void CompleteFork(Ptr<const Packet> p, const TcpHeader& h, const Address& fromAddress, const Address& toAddress);

  virtual void ProcessSynRcvd(Ptr<Packet> packet, const TcpHeader& tcpHeader, const Address& fromAddress,
...
//  virtual bool AddPeerMapping(const MpTcpMapping& mapping);
  ...
  virtual void GetMappedButMissingData(std::set<MpTcpMapping>& missing);
